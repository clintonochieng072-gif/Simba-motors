...
import { getAllCars, deleteCar, updateCar, addCar } from "../lib/api";
...
const AdminCars = () => {
  ...
  const handleFeatureKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === "Enter") {
      e.preventDefault();
      const feature = (e.target as HTMLInputElement).value.trim();
      if (feature) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (
        ...
        (!Array.isArray(formData[key as keyof typeof formData]) ||
          (formData[key as keyof typeof formData] as any[]).length > 0)
      ) {
        if (Array.isArray(formData[key as keyof typeof formData])) {
          ...
          carData.append(key, formData[key as keyof typeof formData] as string);
        }
      }
    });
    ...
    try {
      ...
      toast?.showSuccess("Car added successfully!");
    } catch (error) {
      console.error("Error saving car:", error);
      toast?.showError("Error saving car");
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this car?")) {
      ...
      try {
        ...
        fetchCars();
        toast?.showSuccess("Car deleted successfully!");
      } catch (error) {
        console.error("Error deleting car:", error);
        toast?.showError("Error deleting car");
      }
    }
  };
  ...
  const handleStatusToggle = async (car: any) => {
    ...
    try {
      ...
      await updateCar(car._id, updateData, token);
      fetchCars();
      toast?.showSuccess(`Car ${newStatus.toLowerCase()} successfully!`);
    } catch (error) {
      console.error("Error updating car status:", error);
      toast?.showError("Error updating car status");
    }
  };
  ...
  const filteredAndSortedCars = (cars || [])
    ...
    .sort((a: Car, b: Car) => {
      const aValue = a[sortConfig.key as keyof Car];
      const bValue = b[sortConfig.key as keyof Car];
                ...
                <tr key={car._id} className="hover:bg-neutral-50">
                  ...
                  <td className="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center gap-1 sm:gap-2">
                      <button
                        onClick={() =>
                          router.push(`/admin/dashboard/cars/${car._id}`)
                        }
